...
$urvenue_ws_assetsversion = "1.2.6"; // Axl UWS-7416
...
if (function_exists('get_option')) {//is wordpress
	...
	// $uvlibinfojson = file_get_contents("$urvenue_ws_coredir/uvcore.lib.json");
	...
	if (is_array($urvenue_ws_uvlib["system"])) {
		// $urvenue_ws_libexits = true;
		$urvenue_ws_libexits = true; // Axl UWS-7416
		// $urvenue_ws_path = $uvlib["system"]["path"];
...
if ($urvenue_ws_path) {
	...
	if (!urvenue_ws_is_wordpress())
		// $urvenue_ws_feeds_debug = (isset($_REQUEST["uvdbg"]) and sanitize_text_field( wp_unslash( $_REQUEST["uvdbg"] ) ) and (sanitize_text_field( wp_unslash( $_REQUEST["uvdbg"] ) ) == date("j"))) ? 1 : 0; // Axl UWS-7418
	...
	if (function_exists('get_option') && $urvenue_ws_uvurlpath == '/apis/uvclearcache/') {
		if (FALSE === get_option('urvenue_ws_cacheword') && FALSE === update_option('urvenue_ws_cacheword', FALSE))
			add_option('urvenue_ws_cacheword', ''); // Axl UWS-7416
