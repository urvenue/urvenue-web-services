...
if ( ! isset( $_REQUEST['uws_nonce'] ) ||
     ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['uws_nonce'] ) ), 'urvenue_ws_inventory' ) ) {
...
if(is_array($urvenue_ws_response) and  $urvenue_ws_response["uv"]["success"]["status"] == "success"){
    $urvenue_ws_popcontent = urvenue_ws_get_template("inventory/inventory-inq-success");
}
else{
    $urvenue_ws_errormsg = (isset($urvenue_ws_response["uv"]["success"]["message"])) ? $urvenue_ws_response["uv"]["success"]["message"] : "";
    $urvenue_ws_popcontent = urvenue_ws_get_template("inventory/inventory-inq-failed");
