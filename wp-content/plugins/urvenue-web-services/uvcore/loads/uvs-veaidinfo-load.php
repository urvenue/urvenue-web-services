...
if ( ! current_user_can( 'manage_options' ) ) {
	echo "<div class='uvs-admin-errormsg'>Insufficient permissions.</div>";
	exit;
}
if ( ! isset( $_REQUEST['uws_nonce'] ) ||
     ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['uws_nonce'] ) ), 'uvsp_veaidinfo' ) ) {
	echo "<div class='uvs-admin-errormsg'>Invalid security token.</div>";
	exit;
}
...
$urvenue_ws_adm_ve = isset($urvenue_ws_adm_ve) ? $urvenue_ws_adm_ve : ( isset( $_REQUEST["uvsve"] ) ? sanitize_text_field( wp_unslash( $_REQUEST["uvsve"] ) ) : '' ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Admin load file; admin capability check handles authorization
$urvenue_ws_adm_nv = isset($urvenue_ws_adm_nv) ? $urvenue_ws_adm_nv : ( isset( $_REQUEST["uvsnv"] ) ? sanitize_text_field( wp_unslash( $_REQUEST["uvsnv"] ) ) : '' ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Admin load file; admin capability check handles authorization
...
$urvenue_ws_adm_venueinfofeed = urvenue_ws_adm_pullfeed($urvenue_ws_adm_venueinfofeedurl);
...
if(is_array($urvenue_ws_adm_venueinfofeed) and is_array($urvenue_ws_adm_venueinfofeed["venues"]) and is_array($urvenue_ws_adm_venueinfofeed["venues"][0]) and ($urvenue_ws_adm_venueinfofeed["venues"][0]["id"] == $urvenue_ws_adm_ve)){
	...
	$urvenue_ws_adm_venueforminfo = "<input class='uvsjson' type='hidden' name='venues[{$urvenue_ws_adm_wbcode_esc}][wbcode]' value='{$urvenue_ws_adm_wbcode_esc}'>";
	$urvenue_ws_adm_venueforminfo .= "<input class='uvsjson' type='hidden' name='venues[{$urvenue_ws_adm_wbcode_esc}][venue-name]' value='" . esc_attr( $urvenue_ws_adm_venuename ) . "'>";
	$urvenue_ws_adm_venueforminfo .= "<input class='uvsjson' type='hidden' name='venues[{$urvenue_ws_adm_wbcode_esc}][veaid]' value='" . esc_attr( $urvenue_ws_adm_venueveaid ) . "'>";
	...
	$urvenue_ws_venuealiasinput = "<div class='uvs-infolist-item uvs-clearfix'><div class='uvsname'>Use Alias as Venue Name:</div><div class='uvsvalue'><div class='uvs-switch-ui " . esc_attr( $urvenue_ws_venuealiasswitchclass ) . "'><button class='uvsjs-trigger-switch' type='button'><span class='uvs-lb-on'>Yes</span><span class='uvs-lb-off'>No</span></button><input class='uvsjson' type='hidden' name='venues[{$urvenue_ws_adm_wbcode_esc}][venueforcealias]' value='" . esc_attr( $urvenue_ws_adm_venueforcealias ) . "' data-value-on='1' data-value-off=''></div></div></div>";

	$urvenue_ws_hideeventsswitchclass = ($urvenue_ws_adm_venuehideinevents) ? "uvs-on" : "";
	$urvenue_ws_hideeventsinput = "<div class='uvs-infolist-item uvs-clearfix'><div class='uvsname'>Use Alias as Venue Name:</div><div class='uvsvalue'><div class='uvs-switch-ui " . esc_attr( $urvenue_ws_hideeventsswitchclass ) . "'><button class='uvsjs-trigger-switch' type='button'><span class='uvs-lb-on'>Yes</span><span class='uvs-lb-off'>No</span></button><input class='uvsjson' type='hidden' name='venues[{$urvenue_ws_adm_wbcode_esc}][venuehideinevents]' value='" . esc_attr( $urvenue_ws_adm_venuehideinevents ) . "' data-value-on='1' data-value-off=''></div></div></div>";

	$urvenue_ws_adm_pendchanges_script = 'urvenue_ws_adm_pendchanges = true;';

	wp_register_script('urvenue_ws_adm_pendchanges', false, array(), $urvenue_ws_assetsversion, true);
	wp_enqueue_script('urvenue_ws_adm_pendchanges');
	wp_add_inline_script('urvenue_ws_adm_pendchanges', "(function () { {$urvenue_ws_adm_pendchanges_script} })();");
	...
	echo wp_kses( $urvenue_ws_adm_venueinfoinfohtml, urvenue_ws_adm_allowed_admin_html() );
}
else {
