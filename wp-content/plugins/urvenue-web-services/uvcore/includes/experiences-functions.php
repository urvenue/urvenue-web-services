...
*/
function urvenue_ws_get_experiences_date_filter($uvargs = ""){
    global $urvenue_ws_core_lib, $urvenue_ws_today;

    $uvdate = urvenue_ws_get_arg($uvargs, "date", $urvenue_ws_today);

    $uvinitialddate = gmdate("F j, Y", strtotime($uvdate));
    $uvmaxdate = gmdate("Y-m-d", strtotime($uvdate . " +3 months"));
...
*/
function urvenue_ws_experiences($uvargs = ""){
    global $urvenue_ws_path, $urvenue_ws_today;

    //$uvexperiences = uws_get_dummyapi("experiences");
    $uvdate = urvenue_ws_get_arg($uvargs, "date", $urvenue_ws_today);
    $uvexperiencesfilters = urvenue_ws_get_experiences_filters($uvexperiences, $uvargs);
    $uvinvitems = urvenue_ws_inventory_microcode_items(array("date" => $uvdate));

    //Add real items to dummy filters api --- shound connect the filters latter
    $uvexperiences["items"] = $uvinvitems;

    $uvexperienceslist = urvenue_ws_get_experiences_list($uvexperiences);
    $uvintclass = urvenue_ws_get_arg($uvargs, "customclass", "");
...
*/
function urvenue_ws_related_experiences($uvargs = ""){
    $uvmastercode = urvenue_ws_get_itempagemastercode();//current item page
    $uvexperiences = urvenue_ws_get_dummyapi("experiences");
    $uvnexperiences = urvenue_ws_get_arg($uvargs, "nexperiences", 3);
    $uvintclass = urvenue_ws_get_arg($uvargs, "customclass", "");
    ...
    $uvexperienceslist = urvenue_ws_get_experiences_list($uvexperlist);
...
*/
function urvenue_ws_get_experiences_list($uvexperiences){
    global $urvenue_ws_core_lib;

    $uvexperienceslist = "";

    if(is_array($uvexperiences) and is_array($uvexperiences["items"])){
        $uvexperienceslistitemtemplate = urvenue_ws_get_template("inventory/inventory-experience-list-item");

        foreach($uvexperiences["items"] as $uvitemmascode => $uvitem){
            $uvitemdprice = urvenue_ws_frontformat_money($uvitem["listprice"], 1);
            $uvitemurl = urvenue_ws_get_item_url($uvitem);
            ...
            $uvdstartdtime = ($uvitem["starttime"]) ? urvenue_ws_get_formattime($uvitem["starttime"]) : "";
            $uvdenddtime = ($uvitem["endtime"]) ? urvenue_ws_get_formattime($uvitem["endtime"]) : "";
...
*/
function urvenue_ws_get_experiences_filters($uvexperiences, $uvargs = ""){
    $uvexperiencesfilters = "";
    ...
    if(is_array($uvexperiences)){
        ...
        $uvexpcatslist = urvenue_ws_get_optionslist($uvexpcats);
        $uvexprecolist = urvenue_ws_get_optionslist($uvexpreco, "", "checkboxes");
        $uvexpactylist = urvenue_ws_get_optionslist($uvexpacty, "", "checkboxes");
        $uvexpbudglist = urvenue_ws_get_optionslist($uvexpbudg, "", "checkboxes");
        $uvexptodalist = urvenue_ws_get_optionslist($uvexptoda, "", "checkboxes");
        $uvexpcomplist = urvenue_ws_get_optionslist($uvexpcomp, "", "checkboxes");
